...
import { formatTime, formatTimecode } from '../lib/time';
...
function TimeReadout() {
  ...
  useEffect(() => {
    const apply = () => {
      ...
      if (currentRef.current) {
        currentRef.current.textContent = formatTimecode(s.currentTimeMs, s.project.fps);
      }
      if (totalRef.current) {
        totalRef.current.textContent = formatTimecode(projectDurationMs(s.project), s.project.fps);
      }
    };
    apply();
  ...
  return (
    <span className="min-w-[118px] text-center font-mono text-xs tabular-nums text-zinc-400">
      <span ref={currentRef} className="text-zinc-100" /> / <span ref={totalRef} />
    </span>
  );
}
  ...
  return (
    <div className="flex h-11 flex-none items-center justify-center gap-1 border-y border-zinc-800 bg-zinc-900 px-2">
      ...
      <button
        ...
        title="Loop playback over the selected region (Q)"
      >
      ...
      {region && (
        <button
          ...
          title="Selected region — click to clear"
        >
      ...
      {!coarse && (
        <>
          ...
          <button
            className="rounded-lg p-2 text-zinc-400 active:bg-zinc-800"
            onClick={() => zoomToFit()}
            title="Fit project in view (Shift+Z)"
          >
