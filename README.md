...
- **Variable preview resolution** - the monitor composites at a fraction of the export size (Full / ½ / ¼ / ⅛), picked from the quality menu in the monitor's bottom-right corner. **Auto** (the default) measures the real per-frame composite cost and steps down the ladder when a machine can't keep up, back up when it has headroom - fluid playback on weak hardware without a manual choice. In Auto, a paused still refines to full resolution once the playhead settles (draft while scrubbing, sharp when it stops). The chosen mode persists (`selfcut.previewResolution`); export is unaffected.
